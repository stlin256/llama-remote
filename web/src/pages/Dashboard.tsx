  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid" style={{
        ...
        <div className="panel" style={{ padding: isMobile ? 6 : 8 }}>
          <div className="flex items-center gap-2 mb-2">
            <Server size={isMobile ? 14 : 16} />
            <span className="text-sm">{t('instances')}</span>
          </div>
          <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 'bold' }}>{instances.length}</div>
        </div>
        <div className="panel" style={{ padding: isMobile ? 6 : 8 }}>
          <div className="flex items-center gap-2 mb-2">
          ...
          <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 'bold', color: '#00aa00' }}>{runningInstances.length}</div>
        </div>
        <div className="panel" style={{ padding: isMobile ? 6 : 8 }}>
          <div className="flex items-center gap-2 mb-2">
            <Cpu size={isMobile ? 14 : 16} />
            <span className="text-sm">{t('cpu')}</span>
          </div>
          <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 'bold' }}>{systemStats ? `${systemStats.cpu.toFixed(0)}%` : 'N/A'}</div>
        </div>
        <div className="panel" style={{ padding: isMobile ? 6 : 8 }}>
          <div className="flex items-center gap-2 mb-2">
            <HardDrive size={isMobile ? 14 : 16} />
            <span className="text-sm">{t('memory')}</span>
          </div>
          <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 'bold' }}>{systemStats ? `${systemStats.mem_used.toFixed(0)}GB` : 'N/A'}</div>
        </div>
        <div className="panel" style={{ padding: isMobile ? 6 : 8 }}>
          <div className="flex items-center gap-2 mb-2">
          ...
          <div style={{ fontSize: isMobile ? 18 : 24, fontWeight: 'bold' }}>{gpuStats ? `${gpuStats.memory_used.toFixed(1)}GB` : 'N/A'}</div>
        </div>
      </div>

      {/* System Details - responsive grid */}
      <div className="grid" style={{
      ...
      {/* Instances */}
      <div className="panel" style={{ marginTop: 8, padding: isMobile ? 4 : 8 }}>
